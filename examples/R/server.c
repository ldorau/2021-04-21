...
main(int argc, char *argv[])
{
	...
	if (pmem_ptr == NULL) {
		(void) fprintf(stderr, "Server: pmem_map_file() for %s failed\n", path);
		return -1;
	}

	/* pmem is expected */
	if (!is_pmem) {
		(void) fprintf(stderr, "Server: %s is not an actual PMEM\n", path);
	...
	if (pmem_size < PMEM_MIN_SIZE) {
		(void) fprintf(stderr, "Server: %s too small (%zu < %u)\n",
	...
	(void) printf("Server: the initial content of the server's memory: %s\n", mr_ptr);
	...
	(void) printf("Server: received a new data from the client: %s\n", mr_ptr);
